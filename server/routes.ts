...
export async function registerRoutes(httpServer: HttpServer, app: Express) {
  ...
  // ============================================================
  // Manager-only alpha helper: finalize a draft prescription on the
  // prescriber's behalf. Used when a prescriber forgot to click Sign and the
  // Rx is stuck in draft, blocking the pharmacy queue. Anchors to XRPL
  // Testnet exactly like the prescriber's sign endpoint and records ledger
  // attribution to the manager who unstuck it.
  // ============================================================
  app.post("/api/manager/prescriptions/:id/finalize", requireRole("manager"), async (req: any, res) => {
    try {
      const id = parseInt(req.params.id);
      const rx = await storage.getPrescription(id);
      if (!rx) return res.status(404).json({ error: "Not found" });
      if (rx.status !== "draft") return res.status(400).json({ error: "Only draft prescriptions can be finalized." });
      if (!rx.pharmacyId) return res.status(400).json({ error: "Draft has no destination pharmacy set. Open it in the prescriber workspace and pick a pharmacy first." });

      const patient = await storage.getPatient(rx.patientId);
      const prescriber = await storage.getUserById(rx.prescriberId);
      const manager = await storage.getUserById(req.session.userId);
      const routingChannel = rx.channel || "manual";
      const destinationSoftware = rx.destinationSoftware || "manual";
...
<!-- SIMULATED NCPDP SCRIPT 2017071 NewRx — generated by HunaDoc alpha. Finalized by Operations Manager on behalf of prescriber. -->
<Message xmlns="http://www.ncpdp.org/schema/SCRIPT">
  <Header>
    <To>${softwareLabel}</To>
    <From>HunaDoc Operations Manager (on behalf of ${prescriber?.fullName ?? "prescriber"})</From>
  ...
  <Body>
    <NewRx>
      <Patient><MRN>${patient?.mrn ?? ""}</MRN><Name>${patient?.firstName ?? ""} ${patient?.lastName ?? ""}</Name></Patient>
      <Prescriber><Name>${prescriber?.fullName ?? ""}</Name><NPI>${prescriber?.npi ?? ""}</NPI></Prescriber>
      <Medication><DrugDescription>${rx.drug} ${rx.strength} ${rx.form}</DrugDescription><Sig>${rx.sig}</Sig><Quantity>${rx.quantity}</Quantity><Refills>${rx.refills ?? 0}</Refills></Medication>
      <Pharmacy><PharmacyUserId>${rx.pharmacyId}</PharmacyUserId></Pharmacy>
    </NewRx>
  </Body>
</Message>`;
      const docPayload = {
        ...
        finalized_by_manager_id: req.session.userId,
      };
      ...
      await storage.recordLedger({
        ...
        signerName: `${manager?.fullName ?? "Operations Manager"} (on behalf of ${prescriber?.fullName ?? "prescriber"})`,
        network: "testnet",
        explorerUrl: result.explorerUrl,
      } as any);
      res.json({ ok: true, prescription: updated, broadcast: result });
    } catch (e: any) {
      console.error("[manager.finalize]", e);
      res.status(500).json({ error: e.message });
    }
  });
